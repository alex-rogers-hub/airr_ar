...
#' Score a profile across all of a user's brands and tracked prompts
#' @param login_id Integer
#' @param profile_id Integer
#' @param model Character
score_profile <- function(login_id, profile_id, model = "gpt-4o-mini") {
  ...
  for (b in seq_len(nrow(brands))) {
    ...
    # Score tracked prompts for this brand
    if (nrow(queries) > 0) {
...
#' Score one brand for one profile
score_profile_brand <- function(brand_id, brand_name, profile_id, prefix,
                                brand_reach    = "global",
                                reach_country  = NULL,
                                reach_region   = NULL,
                                reach_postcode = NULL,
  ...
  # Build profile-prefixed presence prompts
  # These are the prompts where appearing = being a brand this profile would choose
  presence_prompts <- c(
  ...
  # Perception prompts — profile-aware factual questions about the brand
  ...
  # Calculate presence — how often brand_name appears in responses
  ...
  # Calculate perception — sentiment from profile-prefixed descriptions
  ...
  # Prestige — rank from presence prompts (position in response)
  prestige_score <- calculate_prestige_from_prompts_sep(
    brand_name, brand_id, all_presence_responses
  )
  ...
  if (nrow(presence_history) < 5) {
    persistence_score <- presence_score
  } else {
    persistence_score <- calculate_daily_persistence_sep(presence_history)
  }
...
#' Score one prompt for one brand for one profile
score_profile_prompt <- function(brand_id, brand_name, profile_id,
  ...
  prestige_score   <- calculate_prestige_from_prompts_sep(brand_name, brand_id, rel_responses)
  perception_score <- calculate_perception_from_prompts_sep(brand_name, brand_id, rel_responses)
